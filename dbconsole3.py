#! /usr/bin/env python3
# -*- coding: utf-8 -*-
# 2005-11-21 by Yüce TEKOL.
# Last update: 2023-09-23

# DB Console
# Copyright (C) 2006-2023  Yuce Tekol
#
...
def print_columns(descs, rows):
	...
	for row in rows:
		for x, r in enumerate(row):
			collens[x] = max(collens[x], len(str(r)))
...
def main():
	...
	if options.sqlcmd:
		if not args:
			print("error: DB file not specified!", file=sys.stderr)
			sys.exit(10)
			...
			if not ";" in cmdin:
				...
				for cmd in cmdstr.split(";"):
					...
					if not cmd:
						continue		
					if not db:
						print("error: DB not open. Use '\open' command.", sys.stderr)
					else:
